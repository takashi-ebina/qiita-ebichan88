...
１度開いたカーソルに関して、カーソルを閉じる前に再度開くことはできませんが、
...
また、`GET STACKED DIAGNOSTICS`を用いて、呼び出し元のECXEPTION内で上記で設定した詳細情報を取得することが可能です。
...
PL/pgSQLでは、**プロシージャに関して**、`COMMIT`および`ROLLBACK`コマンドを用いてトランザクションの制御をすることが可能です。

また、`BEGIN`~`EXCEPTION`のブロック内で例外が発生した場合、
`BEGIN`ブロック内の処理は自動的にロールバックされます。
...
ただし、それ以外に、**無名ブロック**の仕組みを利用することで、**プロシージャやファンクションを実装せずとも、PL/pgSQLの処理の実行が可能**となります。
...
https://www.postgresql.jp/document/12/html/xfunc-sql.html
