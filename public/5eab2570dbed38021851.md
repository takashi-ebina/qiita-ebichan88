...
tags:
  ...
  - TypeScript
private: false
updated_at: '2026-01-04T21:27:36+09:00'
...
本記事はJavaエンジニアがTypeScript扱う際に感じた違いを解説した記事です。
...
実行時に型情報が存在するJavaと異なるため、実行時に型によって処理を分岐する、JavaのようなオーバーロードはTypeScriptでは実現できません。
...
### 補足：TypeScriptでは「実装」ではなく「シグネチャ」をオーバーロードできる
...
[サバイバルTypeScript](https://typescriptbook.jp/reference/values-types-variables/structural-subtyping#%E6%A7%8B%E9%80%A0%E7%9A%84%E5%9E%8B%E4%BB%98%E3%81%91%E3%81%AE%E6%8E%A1%E7%94%A8%E7%90%86%E7%94%B1)によると、JavaScriptの
...
2.で説明した構造的型付けを踏まえて、interfaceの違いについて解説します。
Javaのinterfaceはクラスが実装すべきメソッドのシグネチャを定義します。

```java:Java
void main() {
  Displayable d = new User("Alice", 30);
...
TypeScriptのinterfaceはオブジェクトの形状を定義するために使用され、プロパティやメソッドの型を指定します。

```typescript:TypeScript
function main(d: Displayable) {
...
Javaにおいては`implements`キーワードを用いてクラスがinterfaceを実装する必要があります。

TypeScriptでもインターフェースをクラスに実装させることはできますが、インターフェースと実装関係がないオブジェクトの型注釈としても利用できます。
今回の例では、`user`オブジェクトはDisplayableインターフェースを明示的に実装していませんが、構造的型付けにより問題なく`main`関数に渡すことができます。
...
と記述されているので、基本的には`interface`ではなく`type`を使う方が良さそうです。
...
オプショナルチェーン演算子（`?.`）やNull合体演算子（`??`）を使用して、安全に値を扱うことが推奨されます。
...
TypeScriptではなく、JavaScriptにも当てはまる内容は本記事ではできる限り割愛しています。
